...
class PandaRobotGymEnv(gym.Env):
    ...
    def reset(self):
        ...
        goalx = random.randint(5, 25)/20
        goaly = 0
        goalz = random.randint(4, 16)/20
        ...
        obs = [self.joint_state[1], self.joint_state[3]]
        obs = np.append(obs, vector_dist)
        return np.array(obs)

    def step(self, action):
        self.done = False
        #self.joint1 = self.joint1 + action[0]
        self.joint2 = self.joint2 + action[0] / 20
        #self.joint3 = self.joint3 + action[2]
        self.joint4 = self.joint4 + action[1] / 20
        #self.joint5 = self.joint5 + action[4]
        #self.joint6 = self.joint6 + action[5]
        self.joint7 = self.joint7 + action[6]
        self.set_joint_state()
