...
def take_action(msg):
    ...
    joint_state[1] = joint_state[1] + (msg.action[0] / 20)
    joint_state[3] = joint_state[3] + (msg.action[1] / 20)
    ...
    if joint_state[0] < joint1_threshold_min or joint_state[0] > joint1_threshold_max \
        ...
        obs = [joint_state[1], joint_state[3]]
        obs = np.round(obs, 5)
...
rospy.init_node('step_service_11315', anonymous=False)
print("step_node_11315 aktiv")
s = rospy.Service('step_env_11315', StepAction, take_action)
rospy.spin()
